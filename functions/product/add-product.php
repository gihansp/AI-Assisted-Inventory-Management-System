...
if ($_POST) {
    try {
        ...
        if (!in_array(strtolower($type), $allowedImageFormats)) {
            throw new Exception('Invalid image format. Please upload an image file.');
        }

        if (!is_uploaded_file($_FILES['prod_img']['tmp_name'])) {
            throw new Exception('File not uploaded. Please select a file to upload.');
        }

        if (!move_uploaded_file($_FILES['prod_img']['tmp_name'], $url)) {
            throw new Exception('Failed to move the uploaded file. Please try again later.');
        }
        ...
        if ($connect->query($sql) === TRUE) {
            ...
            $response['updateFeedback'] = "Product successfully added.";
        } else {
            throw new Exception('Failed to add the product. Please try again later.');
        }
    } catch (Exception $e) {
